local library = {
	Version = "0.33",
	WorkspaceName = "Maru Hub Premium Scripts",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
	...
	colors = {
        ...
        selectedOption = Color3.fromRGB(17,10,10),
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Maru Hub Premium Scripts")
							if not fold then
								makefolder("./Maru Hub Premium Scripts")
							end
							fold = isfolder("./Maru Hub Premium Scripts/Themes")
							if not fold then
								makefolder("./Maru Hub Premium Scripts/Themes")
							end
							fold = isfolder("./Maru Hub Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Maru Hub Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Maru Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Maru Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Maru Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
		...
		if library.objects and (#library.objects > 0 or next(library.objects)) then
			...
			pcall(function()
				if library.Backdrop then
					...
					library.Backdrop.ImageTransparency = (library_flags["__Designer.Background.ImageTransparency"] or 65) / 100
				end
			end)
		end
	end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Maru Hub Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "Maru Hub Premium Scripts")
	end
	...
	innerBackdrop.ImageTransparency = (library_flags["__Designer.Background.ImageTransparency"] or 65) / 100
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local common_string = "./Maru Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Maru Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Maru Hub Premium Scripts Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
